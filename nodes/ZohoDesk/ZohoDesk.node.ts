...
/**
 * Format date range for Zoho Desk search API
 * @param createdAfter - Optional start date
 * @param createdBefore - Optional end date
 * @returns Formatted date range string "startDate,endDate" or null if no dates
 */
function formatZohoDateRange(createdAfter?: string, createdBefore?: string): string | null {
  if (!createdAfter && !createdBefore) {
    return null;
  }

  // Format: startDate,endDate (ISO 8601)
  // If only one date provided, use min/max for the other
  const startDate = createdAfter
    ? new Date(createdAfter).toISOString()
    : '1970-01-01T00:00:00.000Z';
  const endDate = createdBefore ? new Date(createdBefore).toISOString() : new Date().toISOString();
...
/**
 * Extract clean reply text from HTML email content
 * Strips quoted email history and returns only the new message content
...
function extractCleanText(htmlContent: string): string {
  ...
  // First convert HTML to plain text, preserving structure
  let text = htmlToText(htmlContent);

  // Now remove quoted content from plain text
  // These patterns match quote markers and everything after them

  const quotePatterns = [
    // Zoho-specific Hebrew format: "---- בתאריך ... ----"
    /\s*----\s*בתאריך[\s\S]*/im,
    // Zoho-specific English format: "---- On ... wrote ----"
    /\s*----\s*On\s+[\s\S]*?wrote\s*----[\s\S]*/im,
    // Standard "On ... wrote:" format
    /\s*On\s+\w{3},\s+\w{3}\s+\d+,\s+\d{4}\s+at\s+[\s\S]*?wrote:[\s\S]*/im,
    /\s*On\s+\d{1,2}[\/-]\d{1,2}[\/-]\d{2,4}[\s\S]*?wrote:[\s\S]*/im,
    // German format: "... hat am DD.MM.YYYY ... geschrieben:"
    /\s*[^\n]+hat am \d{1,2}\.\d{1,2}\.\d{4}[^\n]*geschrieben:[\s\S]*/im,
    // Outlook format: "From: ... Sent: ..."
    /\s*From:\s+[^\n]+\n\s*Sent:\s+[\s\S]*/im,
    // Generic "Original Message" marker
    /\s*-{3,}\s*Original Message\s*-{3,}[\s\S]*/im,
    // CSS selector for original sender (Zoho specific)
    /#original_sender\s*\{[^}]+\}/gim,
  ];
      ...
      },
      {
        ...
        options: [
          ...
          },
          {
            ...
            default: '',
            description:
              'Filter tickets created on or after this date. Requires Department ID when used.',
          },
          {
            ...
            default: '',
            description:
              'Filter tickets created on or before this date. Requires Department ID when used.',
          },
          {
          ...
          },
          {
            ...
            options: [
              ...
              { name: 'Customer Response Time', value: 'customerResponseTime' },
        ...
        options: [
          {
            ...
            default: false,
            description:
              'Whether to fetch clean message content for each thread. Returns a cleanText field with quoted email history removed. (makes additional API calls)',
          },
          {
            displayName: 'Sort By',
            name: 'sortBy',
        ...
        if (resource === 'ticket') {
          ...
          if (operation === 'list') {
            ...
            if (filters.createdAfter && filters.createdBefore) {
              const afterDate = new Date(filters.createdAfter as string);
              const beforeDate = new Date(filters.createdBefore as string);
            ...
            // Require departmentId when date filters are used
            if ((filters.createdAfter || filters.createdBefore) && !filters.departmentId) {
              throw new NodeOperationError(
                this.getNode(),
                'Department ID is required when using date filters. Please select a department in the Filters section.',
              );
            }
            ...
            if (filters.sortBy) {
              ...
              queryParams.sortBy = `${sortOrder}${filters.sortBy}`;
            }

            // Determine if we need to use search endpoint (for date filtering)
            const useDateFilters = !!(filters.createdAfter || filters.createdBefore);

            // Build search-specific params if using date filters
            const searchParams: IDataObject = {};
            if (useDateFilters) {
              const dateRange = formatZohoDateRange(
                filters.createdAfter as string | undefined,
                filters.createdBefore as string | undefined,
              );
              if (dateRange) {
                searchParams.createdTimeRange = dateRange;
              }
              // Search endpoint requires departmentId
              searchParams.departmentId = filters.departmentId;
              // Add other filters to search params
              if (filters.assigneeId) searchParams.assigneeId = filters.assigneeId;
              if (filters.status) searchParams.status = filters.status;
              if (filters.sortBy) {
                const sortOrder = filters.sortOrder === 'desc' ? '-' : '';
                searchParams.sortBy = `${sortOrder}${filters.sortBy}`;
              }
            }

            // Determine which endpoints to call based on ticketType and date filters
            const endpoints: string[] = [];
            if (useDateFilters) {
              // When date filters are used, we must use the search endpoint
              endpoints.push('/tickets/search');
            } else {
            ...
            if (returnAll) {
              ...
              for (const endpoint of endpoints) {
                const params = useDateFilters ? searchParams : queryParams;
              ...
              for (const endpoint of endpoints) {
                ...
                const options = {
                  ...
                  headers: { orgId },
                  uri: `${baseUrl}${endpoint}`,
                  qs: { from: 1, limit: limitPerEndpoint, ...params },
                  json: true,
                };
          ...
          if (operation === 'listThreads') {
            const ticketId = this.getNodeParameter('ticketId', i) as string;
            const returnAll = this.getNodeParameter('returnAll', i) as boolean;
            const threadOptions = this.getNodeParameter('options', i) as IDataObject;
            const includeFullContent = threadOptions.includeFullContent as boolean;
            ...
            let threads: IDataObject[] = [];

            if (returnAll) {
              threads = await getAllPaginatedItems(
                this,
                baseUrl,
            ...
            // If includeFullContent is enabled, fetch full content for each thread
            if (includeFullContent && threads.length > 0) {
              for (const thread of threads) {
                ...
                if (threadId) {
                  const threadOptions = {
                    method: 'GET' as const,
                    headers: { orgId },
                    uri: `${baseUrl}/tickets/${encodeURIComponent(ticketId)}/threads/${encodeURIComponent(threadId)}`,
                    json: true,
                  };
                  ...
                  thread.cleanText = extractCleanText(htmlContent);
                }
              }
            }

            // Sort threads if requested
            const sortBy = threadOptions.sortBy as string;
            const sortOrder = threadOptions.sortOrder as string;
            if (sortBy && threads.length > 1) {
              threads.sort((a, b) => {
